...
export const WalletDbChangeCallbacks: DbChangeCallbacks<WalletFromModel, WalletEntity> = {
	...
	updated: async ({ after, before, changes }) => {
		...
		if (before.subscription.current?.jobId !== after.subscription.current?.jobId && before.subscription.current?.jobId) removedJobIds.push(before.subscription.current.jobId)
		...
		if (changes.subscriptions) {
			...
			const newClasses = newSubs.map((s) => {
				if (s.data.type === 'classes') return { organizationId: s.data.organizationId, classId: s.data.classId }
				return null!
			}).filter(Boolean)

			const oldClasses = oldSubs.map((s) => {
				if (s.data.type === 'classes') return { organizationId: s.data.organizationId, classId: s.data.classId }
				return null!
			}).filter(Boolean)

			await Promise.all([
				...newClasses.map(({ organizationId, classId }) => ClassesUseCases.manageMembers({
					organizationId, classId, userIds: [after.userId], add: true, type: 'students',
				})),
				...oldClasses.map(({ organizationId, classId }) => ClassesUseCases.manageMembers({
					organizationId, classId, userIds: [after.userId], add: false, type: 'students',
				})),
			])
		}

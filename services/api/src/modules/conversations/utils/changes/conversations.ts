...
export const ConversationDbChangeCallbacks: DbChangeCallbacks<ConversationFromModel, ConversationEntity> = {
	...
	updated: async ({ after, before, changes }) => {
		...
		if (justAccepted && after.tutor) {
			if (after.accepted.is) await UsersUseCases.updateTutorConversations({
				userId: after.tutor.id, conversationId: after.id, add: true
			})
			else await WalletsUseCases.updateSubscriptionData({ userId: after.user.id, key: PlanDataType.tutorAidedConversations, value: 1 })
			...
			await sendNotification([after.user.id], {
				...
				data: {
					...
					accepted: after.accepted.is, tutorId: after.tutor.id, conversationId: after.id
				}
			})
		}

		const justEnded = changes.ended && !before.ended && after.ended
		if (justEnded && before.tutor) await Promise.all([
			UsersUseCases.updateTutorConversations({ userId: before.tutor.id, conversationId: after.id, add: false }),
			ReviewsUseCases.add({
				user: after.user, rating: after.ended.rating, message: after.ended.message,
		...
		if (changes.pending && before.pending && !after.pending && !before.accepted && after.accepted && after.tutor) {
			if (!after.accepted) await WalletsUseCases.updateSubscriptionData({ userId: after.user.id, key: PlanDataType.tutorAidedConversations, value: 1 })
			await sendNotification([after.user.id], {
				title: `Tutor request ${after.accepted ? 'accepted' : 'declined'}`,
				body: after.accepted ? `Your tutor request has been accepted by ${after.tutor.bio.name}` : `Your tutor request has been declined by ${after.tutor.bio.name}. You can send a new request to a different tutor`,
				sendEmail: true,
	...
	deleted: async ({ before }) => {
		...
		if (before.tutor && before.accepted?.is) await UsersUseCases.updateTutorConversations({
			userId: before.tutor.id, conversationId: before.id, add: false
		})

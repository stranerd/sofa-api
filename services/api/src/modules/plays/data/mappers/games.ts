...
export class GameMapper extends BaseMapper<GameFromModel, GameToModel, GameEntity> {
	mapFrom (model: GameFromModel | null) {
		...
		const { _id, quizId, user, status, participants, questions, totalTimeInSec, scores, startedAt, endedAt, createdAt, updatedAt } = model
		return new GameEntity({
			id: _id.toString(), quizId, user, status, participants, questions, totalTimeInSec,

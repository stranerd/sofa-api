...
export const QuestionDbChangeCallbacks: DbChangeCallbacks<QuestionFromModel, QuestionEntity> = {
	created: async ({ after }) => {
		await appInstance.listener.created([
			`study/quizzes/${after.quizId}/questions`,
			`study/quizzes/${after.quizId}/questions/${after.id}`,
		], after)
	...
	deleted: async ({ before }) => {
		await appInstance.listener.deleted([
			`study/quizzes/${before.quizId}/questions`,
			`study/quizzes/${before.quizId}/questions/${before.id}`,
		], before)

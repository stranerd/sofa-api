...
export const verifyToBeSaved = async (type: FolderSaved, propIds: string[]) => {
	...
	const { results } = type === FolderSaved.quizzes ? await QuizzesUseCases.get(query) :
		type === FolderSaved.courses ? await CoursesUseCases.get(query) :
			type === FolderSaved.files ? await FilesUseCases.get(query) : { results: [] }

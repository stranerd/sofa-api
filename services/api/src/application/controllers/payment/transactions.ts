...
export class TransactionsController {
	...
	static async create (req: Request) {
		...
		const { amount, data: { type } } = validate({
			amount: Schema.number().gte(100).requiredIf(() => isFundWallet),
			data: Schema.object({
		...
		return await TransactionsUseCases.create({
			...dynamics, currency: wallet.balance.currency, userId: authUser.id, email: authUser.email,
	...
	static async fulfill (req: Request) {
		const transaction = await TransactionsUseCases.find(req.params.id)
		if (!transaction || transaction.userId !== req.authUser!.id || transaction.status !== TransactionStatus.initialized) throw new NotAuthorizedError()

...
export class GameController {
	...
	static async create(req: Request) {
		...
		const questionIds = hasAccess.questions.map((id) => questions.find((q) => q.id === id)?.id).filter((q) => !!q) as string[]
		...
		if (data.join)
			await GamesUseCases.join({ id: game.id, userId: game.user.id, join: true }).then(() => game.participants.push(game.user.id))
